...
export default function RegisterPage() {
  ...
  if (success) return (
    <div className="p-6 max-w-lg mx-auto mt-12 text-center animate-fade-slide-up">
      <div className="gym-card p-10">
        ...
        <p className="text-sm mb-6" style={{ color: 'var(--text-secondary)' }}>{form.full_name} has been added to the system.</p>
        <div className="flex gap-3 justify-center">
          <button onClick={() => { setSuccess(false); setForm({ full_name: '', father_name: '', phone_country_code: '+92', phone_number: '', email: '', address: '', gender: 'Male', fee_amount: 3000, registration_date: today }) }} className="btn-green">
            Add Another
          </button>
          <button onClick={() => router.push('/dashboard/members')} className="btn-primary">
            View Members
          </button>
        </div>
      </div>
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto animate-fade-in">
      <div className="mb-6">
        <h1 className="text-3xl" style={{ fontFamily: 'Rajdhani', fontWeight: 700 }}>ADD NEW MEMBER</h1>
        <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Fill in the member details below</p>
      </div>

      <div className="gym-card p-7">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <input className="gym-input" placeholder={form.phone_country_code === '+92' ? '3001234567 (without leading 0)' : 'Phone number'}
                value={form.phone_number} onChange={e => set('phone_number', e.target.value)} required />
            </div>
            <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
              {form.phone_country_code === '+92' ? 'Pakistan: Enter number without leading 0 (e.g. 3001234567)' : 'Enter number without country code'}
            </p>
          </div>
          ...
          <div>
            ...
            <div className="flex justify-between text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
              <span>PKR 1,500</span><span style={{ color: '#39FF14', fontWeight: 600 }}>PKR {form.fee_amount.toLocaleString()}</span><span>PKR 5,000</span>
            </div>
          </div>
