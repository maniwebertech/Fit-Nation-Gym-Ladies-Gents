  ...
  return (
    <div className="modal-overlay animate-fade-in" onClick={onClose}>
      <div
        ...
        style={{ maxWidth: 680, maxHeight: '92vh', overflow: 'hidden' }}
                ...
                <table className="gym-table">
                  <thead>
                    <tr>
                      ...
                      <th style={{ background: 'var(--bg-card2)', padding: '0.6rem 1rem' }}>NOTES</th>
                    ...
                    {paginated.map((p, i) => {
                      ...
                      return (
                        <tr key={p.id}>
                          ...
                          <td style={{ padding: '0.75rem 1rem' }}>
                            <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>{p.notes || '—'}</span>
                          </td>
                          <td style={{ padding: '0.75rem 1rem' }}>
                            <button
                  ...
                  return (
                    <div key={p.id} className="flex items-center justify-between gap-3 px-5 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-semibold text-sm">{formatDate(p.payment_date)}</div>
                          {p.notes && <div className="text-xs" style={{ color: 'var(--text-muted)' }}>{p.notes}</div>}
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        <span style={{ color: '#39FF14', fontFamily: 'Rajdhani', fontWeight: 700, fontSize: '0.95rem' }}>
                          {formatPKR(p.amount)}
                        </span>
                        <button
                          onClick={() => deletePayment(p.id)}
